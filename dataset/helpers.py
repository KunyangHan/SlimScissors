...
def thin_center_point(thin_pad, labels, ret):
    [h,w] = thin_pad.shape

    map_xor = (thin_pad > 128)
    map_xor_new = np.zeros((h+2, w+2))
    map_xor_new[1:(h+1), 1:(w+1)] = map_xor[:, :]
    distance_transform = ndimage.distance_transform_edt(map_xor_new)
    distance_transform_back = distance_transform[1:(h+1),1:(w+1)] 
    ...
    for l in range(1, ret):
        region_ = np.zeros_like(distance_transform_back)
        region_[labels == l] = distance_transform_back[labels == l]
        raw, column = getPositon(region_)
        center_ = [column, raw]

        thin_center_list.append(center_)

    return thin_center_list


def thin_bg_point(thin_bg, labels, ret):
    bg_list = []
    for l in range(1, ret):
        region_ = np.zeros_like(thin_bg)
        region_[labels == l] = thin_bg[labels == l]
        idx = np.argwhere(region_ > 128)
        if len(idx) > 0:
            random_idx = np.random.choice(len(idx))

            raw, column = idx[random_idx]
            bg_list.append([column, raw])
    
    return bg_list


def thin_bg_point_dilate(thin_pad_bg_list):
    bg_list = []
    for pad in thin_pad_bg_list:
        idx = np.argwhere(pad > 128)
        if len(idx) > 0:
            random_idx = np.random.choice(len(idx))
            raw, column = idx[random_idx]
            bg_list.append([column, raw])
...
def make_thin_points(iog_point, thin_center_list, thin_bg_list, sigma):
    gt_0 = iog_point[:, :, 0]
    gt_1 = iog_point[:, :, 1]
    h, w = gt_0.shape

    for center in thin_center_list:
        gt_0 = np.maximum(gt_0, make_gaussian((h, w), center=center, sigma=sigma))
    for background in thin_bg_list:
        gt_1 = np.maximum(gt_1, make_gaussian((h, w), center=background, sigma=sigma))
