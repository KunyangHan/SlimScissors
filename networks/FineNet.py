...
class FineNet(nn.Module):
    def __init__(self, lateral_channel, out_shape, num_class):
        ...
        self.predict_bottleneck =  Bottleneck(4*lateral_channel, 128)
        self.predict_conv = nn.Conv2d(256, num_class, kernel_size=3, stride=1, padding=1, bias=False)
        self.predict_bn = nn.BatchNorm2d(num_class)
    ...
    def forward(self, x):
        ...
        hid_256 =  self.predict_bottleneck(out)
        out = self.predict_conv(hid_256)
