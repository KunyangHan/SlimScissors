...
class Refiner(nn.Module):
    ...
    def __init__(self,
        ...
        # channels = [32, 64, 64, 64, 1]
        # self.conv1 = nn.Conv2d(channels[0] + 6 + 4, channels[1], kernel_size, bias=False)
        input_channel = refine_in_channels

        self.conv1 = nn.Conv2d(channels[0] + input_channel + 1, channels[1], kernel_size, bias=False)
        self.bn1 = nn.BatchNorm2d(channels[1])
        self.conv2 = nn.Conv2d(channels[1], channels[2], kernel_size, bias=False)
        self.bn2 = nn.BatchNorm2d(channels[2])
        self.conv3 = nn.Conv2d(channels[2] + input_channel, channels[3], kernel_size, bias=False)
        self.bn3 = nn.BatchNorm2d(channels[3])
        self.conv4 = nn.Conv2d(channels[3], channels[4], kernel_size, bias=True)
        self.relu = nn.ReLU(True)
        self.hid_conv1 = nn.Conv2d(channels[0], channels[3], 1)
        self.hid_conv2 = nn.Conv2d(channels[3], channels[3], 3)

        print("Refiner in channel", input_channel)
        if self.mode == 'full':
            print("refiner mode full")
    ...
    def forward(self,
        ...
        if self.mode != 'full':
            ...
            if idx[0].size(0) > 0:
                ...
                hid_12 = self.hid_conv1(hid)
                hid_x4 = self.hid_conv2(x4)
                # hid_12 = F.interpolate(hid_12, (H_full, W_full), mode='bilinear', align_corners=False)
                hid_12 = self.replace_patch(hid_12, hid_x4, idx)
