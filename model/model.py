...
class Base(nn.Module):
    ...
    def __init__(self, backbone: str, in_channels: int, out_channels: int):
        super().__init__()
        assert backbone in ["resnet18", "resnet50", "resnet101", "mobilenetv2"]
        if backbone in ['resnet50', 'resnet101']:
            ...
            self.backbone = ResNetEncoder(in_channels, variant=backbone)
            self.aspp = ASPP(512, [3, 6, 9])
            self.decoder = Decoder([256, 128, 64, 48, out_channels], [128, 64, 64, in_channels])
        else:
...
class MattingBase(Base):
    ...
    def __init__(self, backbone: str, in_channels: int, out_channels=(1 + 3 + 1 + 32)):
        super().__init__(backbone, in_channels=in_channels, out_channels=out_channels)
